...
function spawnpedsroam(coords, model, roamDist)
    if not coords then
        if RanchData and RanchData.ranchcoords and RanchData.ranchcoords ~= 'none' then
    ...
    -- Check if the coords has valid x, y, z fields
    if not coords.x or not coords.y or not coords.z then
        print("Error: Invalid coordinates for spawning animals.")
        return nil
    end
