...
RegisterNetEvent('bcc-ranch:FeedAnimals', function(animalType)
    ...
    local selectAnimalFuncts = {
        ...
        ['sheeps'] = function()
            ...
            if tonumber(RanchData.sheeps_age) < Config.animalSetup.sheeps then
                scale = 0.5
            end
        end,
        ['goats'] = function()
            ...
            if tonumber(RanchData.goat_coords) < Config.animalSetup.goats then
                scale = 0.5
            end
        end,
        ['chickens'] = function()
            ...
            if tonumber(RanchData.chicken_coords) < Config.animalSetup.chickens then
                scale = 0.5
            end
        end
    }
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then isDead = true break end
    end
    ...
    delPedsForTable(feedPeds)
    TriggerServerEvent('bcc-ranch:IncreaseAnimalsCond', RanchData.ranchid, animalType, tables.feedAnimalCondIncrease)
